...
#' @param ddf the method for computing the degrees of freedom. ddf="Satterthwaite" (default) uses Satterthwaite’s method; 
...
#' 
groupComparisonTMT.v2 = function(
    data, formula, 
    ...
    getOption("MSstatsTMTLog")("INFO", paste("Computing the degrees of freedom :", ddf))
    ...
    summarized = MSstatsPrepareForGroupComparisonTMT(data, 
                                                     remove_norm_channel,
                                                     remove_empty_channel)
    contrast_matrix = MSstats::MSstatsContrastMatrix(contrast.matrix,
                                                        unique(summarized$Group))
    fitted_models = MSstatsFitComparisonModelsTMT(summarized, formula, ddf)
...
#' @keywords internal
#' 
MSstatsFitComparisonModelsTMT = function(input, formula, ddf) {
...
#' @keywords internal
#'  
MSstatsComparisonModelSingleTMT = function(single_protein, formula, protein_name, ddf) {
    ...
    fitted_model = .fitModelTMT(single_protein, formula, has_single_subject, 
                                has_techreps, has_biomixtures, has_single_run, 
                                has_Repeated_Measures)
    result = .addVarianceInformation(fitted_model, protein_name, ddf)
    result
}
