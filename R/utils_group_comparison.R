...
#' @keywords internal
.fitModelTMT = function(single_protein, formula, has_single_subject, has_techreps, 
    ...
    if(has_Repeated_Measures){ # time course design
      ...
      if (has_biomixtures) { # multiple mixtures
        if (has_techreps) { # multiple tech MS runs
          ...
          fit = suppressMessages(try(lmerTest::lmer(formula, data = single_protein), TRUE)) # fit the model with subject effect
          if (inherits(fit, "try-error")) { # subject model not applicable - fit one-way anova model
            fit = fit_Group_model(single_protein) 
          }
        }
        ...
        if (has_techreps) { # multiple tech MS runs
          ...
          fit = suppressMessages(try(lmerTest::lmer(formula, data = single_protein), TRUE))
          if (inherits(fit, "try-error")) { # model not applicable - fit one-way anova model
            fit = fit_Group_model(single_protein) 
          }
        }
...
#' @keywords internal
.addVarianceInformation = function(fitted_model, protein_name, ddf) {
