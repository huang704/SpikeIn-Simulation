...
data <- data %>% dplyr::select(Mixture, Channel, BioReplicate, Condition) %>% 
  ...
  dplyr::mutate(Subject_within_mixture = row_number()) %>%
  dplyr::right_join(data) %>% 
  dplyr::mutate(BioReplicate = paste(Mixture, Subject_within_mixture, sep="_")) %>% 
  dplyr::select(-Subject_within_mixture)
...
for(i in seq_along(sim_sub_sd)){
  ...
  save(data.sim, file = paste0("simulation-experiments-unbalanced/controlledmixtures-unbalanced-sub_sd=", sim_sub_sd[i], ".abun.rda"))
...
for(i in seq_along(sim_sub_sd)){
  load(paste0("simulation-experiments-unbalanced/controlledmixtures-unbalanced-", "sub_sd=", sim_sub_sd[i], ".abun.rda"))
  ...
  for(k in seq_along(model_list)){
    ...
    filename1 <- paste0('simulation-experiments-unbalanced/controlledmixtures-unbalanced-sub_sd=', sim_sub_sd[i], '-by-moderated', methods[k], ".testing.rda")
    ...
    filename2 <- paste0('simulation-experiments-unbalanced/controlledmixtures-unbalanced-sub_sd=', sim_sub_sd[i], '-by-moderated', methods[k], ".testing-EB.rda")
  ...
  data.res <- groupComparison(data = as.data.frame(data.sim),
                              ...
                              moderated = TRUE,
                              adj.method = "BH")
  filename4 <- paste0('simulation-experiments-unbalanced/controlledmixtures-unbalanced-sub_sd=', sim_sub_sd[i], '-by-limmaCS-testing.rda')
  ...
  data.res <- groupComparison(data = as.data.frame(data.sim),
                              ...
                              moderated = TRUE, 
                              adj.method = "BH")
  filename5 <- paste0('simulation-experiments-unbalanced/controlledmixtures-unbalanced-sub_sd=', sim_sub_sd[i], '-by-limmaCMS.testing.rda')
