...
for(j in seq_along(sim_sub_sd)){
  ...
  for(i in seq_along(methods)){
    if(i <= 4){
      load(paste0('simulation-experiments/controlledmixtures-sub_sd=', sim_sub_sd[j], '-by-', methods[i], ".testing-EB.rda"))
      data.res <-data.res$ComparisonResult
    }
    else{
      load(paste0('simulation-experiments/controlledmixtures-sub_sd=', sim_sub_sd[j], '-by-', methods[i], ".testing.rda"))
    }
...
write.csv(all_data_summary, file =  paste0("simulation-experiments/res.csv")) 

all_data_summary <- all_data_summary %>% filter(Method != "moderatedTMS")
...
pdf("simulation-experiments/Compare_different_sim_vars_TPs.pdf",width=3.5,height=3)
...
pdf("simulation-experiments/Compare_different_sim_vars_FPs.pdf",width=3.5,height=3)
...
pdf("simulation-experiments/Compare_different_sim_vars_FPs_v2.pdf",width=3.5,height=3)
g4 <- all_data_summary %>% ggplot(aes(x=Sub_sd, y= qvalueFPs, group=Method, color = Method, shape = Method, size = Method))+
  geom_line(size=1)+
  geom_point() +
  scale_size_manual(values=c(3, 2, 2, 2, 2))+
...
pdf("simulation-experiments/Compare_different_sim_vars_FDR.pdf",width=3.5,height=3)
...
pdf("simulation-experiments/Compare_different_sim_vars_sensitivity.pdf",width=3.5,height=3)
...
pdf("simulation-experiments/Compare_different_sim_vars_specificity.pdf",width=3.5,height=3)
...
pdf("simulation-experiments/Compare_different_sim_vars_auc.pdf",width=3.5,height=3)
...
all_data_summary[all_data_summary$Method == "moderatedTS", "Method"] <- "moderatedTS (proposed)"
all_data_summary <- within(all_data_summary, Method <- factor(Method,  
                                                              levels = c("moderatedTS (proposed)", "moderatedTM", "moderatedT", "limmaTS", "limmaTMS")))
pdf("simulation-experiments/Compare_different_sim_vars_legend_new.pdf",width=7,height=4)
g4 <- all_data_summary %>% ggplot(aes(x=Sub_sd, y= fdr, group=Method, color = Method, shape = Method))+
  geom_line(size=1)+
  geom_point() +
  scale_shape_manual(values=c(15, 16, 16, 16, 16))+
  scale_color_manual(values=colors) + 
...
res1 <- all_data
res1[res1$Label == "0.667-0.125", "log2FC"] <- 
  -res1[res1$Label == "0.667-0.125", "log2FC"]
res1[res1$Label == "0.667-0.125", "Label"] <- "0.125-0.667"

res1[res1$Label == "0.667-0.5", "log2FC"] <- 
  -res1[res1$Label == "0.667-0.5", "log2FC"]
res1[res1$Label == "0.667-0.5", "Label"] <- "0.5-0.667"

res1[res1$Label == "0.125-0.5", "log2FC"] <- 
  -res1[res1$Label == "0.125-0.5", "log2FC"]
res1[res1$Label == "0.125-0.5", "Label"] <- "0.5-0.125"

res1[res1$Label == "1-0.5", "log2FC"] <- 
  -res1[res1$Label == "1-0.5", "log2FC"]
res1[res1$Label == "1-0.5", "Label"] <- "0.5-1"

res1[res1$Label == "0.125-1", "log2FC"] <- 
  -res1[res1$Label == "0.125-1", "log2FC"]
res1[res1$Label == "0.125-1", "Label"] <- "1-0.125"

res1[res1$Label == "0.667-1", "log2FC"] <- 
  -res1[res1$Label == "0.667-1", "log2FC"]
res1[res1$Label == "0.667-1", "Label"] <- "1-0.667"

unique(res1$Label)

res1[res1$Label == "0.125-0.667", "Label"] <- "0.125 vs 0.667"

res1[res1$Label == "0.5-0.667", "log2FC"] <- 
  -res1[res1$Label == "0.5-0.667", "log2FC"]
res1[res1$Label == "0.5-0.667", "Label"] <- "0.667 vs 0.5"

res1[res1$Label == "0.5-1", "Label"] <- "0.5 vs 1"

res1[res1$Label == "0.5-0.125", "log2FC"] <- 
  -res1[res1$Label == "0.5-0.125", "log2FC"]
res1[res1$Label == "0.5-0.125", "Label"] <- "0.125 vs 0.5"

res1[res1$Label == "1-0.125", "log2FC"] <- 
  -res1[res1$Label == "1-0.125", "log2FC"]
res1[res1$Label == "1-0.125", "Label"] <- "0.125 vs 1"

res1[res1$Label == "1-0.667", "log2FC"] <- 
  -res1[res1$Label == "1-0.667", "log2FC"]
res1[res1$Label == "1-0.667", "Label"] <- "0.667 vs 1"

unique(res1$Label)

save(res1, file = "simulation-experiments/res1.rda")

res1 <- res1 %>% filter(Method != "moderatedTMS" & Label == "0.125 vs 0.5")
...
pdf("simulation-experiments/Compare_different_sim_vars_se.pdf",width=11,height=4)
...
pdf("simulation-experiments/Compare_different_sim_vars_logfc.pdf",width=11,height=4)
...
pdf("simulation-experiments/Compare_different_sim_vars_DF.pdf",width=11,height=4)
