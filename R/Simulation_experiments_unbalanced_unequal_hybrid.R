...
data <- data %>% dplyr::select(Mixture, Channel, BioReplicate, Condition) %>% 
  unique() %>% 
...
# make the design unbalanced by removing two channels from each mixture
data <- data %>%
  filter(!(Run=='161117_SILAC_HeLa_UPS1_TMT10_Mixture1_01raw' & Channel%in% c("129N", "128C") |
             Run=='161117_SILAC_HeLa_UPS1_TMT10_Mixture2_01raw' & Channel%in% c("128C", "129N") |
             Run=='161117_SILAC_HeLa_UPS1_TMT10_Mixture3_01raw' & Channel%in% c("129C", "128N") |
             Run=='161117_SILAC_HeLa_UPS1_TMT10_Mixture4_01raw' & Channel%in% c("128N", "129C")|
             Run=='161117_SILAC_HeLa_UPS1_TMT10_Mixture5_01raw' & Channel%in% c("127C", "128C")))
...
for(i in seq_along(sim_sub_sd)){
  ...
  save(data.sim, file = paste0("simulation-experiments-unbalanced-hybrid/controlledmixtures-unbalanced-hybrid-sub_sd=", sim_sub_sd[i], ".abun.rda"))
...
for(i in seq_along(sim_sub_sd)){
  load(paste0("simulation-experiments-unbalanced-hybrid/controlledmixtures-unbalanced-hybrid-sub_sd=", sim_sub_sd[i], ".abun.rda"))
  ...
  for(k in seq_along(model_list)){
    ...
    filename1 <- paste0('simulation-experiments-unbalanced-hybrid/controlledmixtures-unbalanced-hybrid-sub_sd=', sim_sub_sd[i], '-by-moderated', methods[k], ".testing.rda")
    ...
    filename2 <- paste0('simulation-experiments-unbalanced-hybrid/controlledmixtures-unbalanced-hybrid-sub_sd=', sim_sub_sd[i], '-by-moderated', methods[k], ".testing-EB.rda")
  ...
  data.res <- groupComparison(data = as.data.frame(data.sim),
                              ...
                              moderated = TRUE,
                              adj.method = "BH")
  filename4 <- paste0('simulation-experiments-unbalanced-hybrid/controlledmixtures-unbalanced-hybrid-sub_sd=', sim_sub_sd[i], '-by-limmaCS-testing.rda')
  ...
  data.res <- groupComparison(data = as.data.frame(data.sim),
                              ...
                              moderated = TRUE,
                              adj.method = "BH")
  filename5 <- paste0('simulation-experiments-unbalanced-hybrid/controlledmixtures-unbalanced-hybrid-sub_sd=', sim_sub_sd[i], '-by-limmaCMS.testing.rda')
...
for(i in seq_along(sim_sub_sd)){
  ...
  save(varscomp, file = paste0('simulation-experiments-unbalanced-hybrid/controlledmixtures-sub_sd=', sim_sub_sd[i], '-CS-variance-comp.rda'))
}